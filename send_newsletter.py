...
- 전체를 감싸는 큰 div 태그는 절대 사용하지 말고, 아래 요구사항에 명시된 개별 태그 규칙을 엄격히 따라줘.

[요구사항]
1. 제목: 주간/일간 뉴스레터 감성을 살려 <h2> 태그로 매력적인 기술 제목을 뽑아줘. (별도의 인라인 스타일은 넣지 마)
2. 서론 (인삿말/자아 표출 금지): 
   - "안녕하세요", "멘토 M입니다", "예비 개발자 여러분" 같은 피상적인 인삿말이나 수식어는 절대 사용하지 마.
   - AI 특유의 응원 멘트나 마무리 감성 문구도 전면 배제해줘.
   - 글의 시작은 곧바로 <p style="font-size: 15.5px; line-height: 1.7; color: #333333;"> 태그를 사용하여, 오늘 다룰 키워드(예: 배열)가 하드웨어(CPU 캐시, 메모리 구조) 관점이나 백엔드 실무(빅데이터 처리, 컬렉션 프레임워크 프레임 등) 아키텍처에서 왜 치명적으로 중요한지 기술적 본질만 담백하고 묵직하게 짚으면서 시작해줘.
3. 본문 요약 및 심화 해설: 
   - 원리를 친절하게 보완해서 풍부하게 설명하되, 모든 일반 단락은 반드시 <p style="font-size: 15.5px; line-height: 1.7; color: #333333;"> 태그를 사용해줘.
   - 중요 키워드는 <strong> 태그를 쓰고, 강조할 문장은 블록 인용구(<blockquote>) 태그를 써줘.
4. 기술 면접 대비 예상 질문 & 모범 답안: 
   - <ul> 태그 내부에 리스트를 구성하되, 각 항목은 반드시 <li style="font-size: 15.5px; line-height: 1.7; color: #333333; margin-bottom: 8px;"> 태그를 활용해 정리해줘.
