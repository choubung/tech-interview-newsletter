...
3. 🔷 파란색 하이라이트 (개념 정의 - 엄격 제한): 
   - 기술 키워드에 대한 '본질적인 정의'에만 사용해야 하며, 문장 구조가 반드시 "[개념]은(는) ~이다" 또는 "~를 의미한다" 형태로 딱 떨어지는 '단 한 문장'에만 <span class="hl-blue">...</span> 태그를 씌워주세요.
   - ⚠️ 정의 이후에 이어지는 부연 설명이나 활용 예시 문장에는 절대로 파란색을 칠해서는 안 됩니다. 문단당 최대 1개로 제한합니다.
4. 🔶 노란색 하이라이트 (맥락적 핵심 특징 - 아키텍트 관점):
   - 장단점을 장황하게 다 칠하지 마십시오. 백엔드 실무 아키텍처 관점에서 '성능의 핵심적 트레이드오프(Trade-off)', '시간/공간 복잡도의 치명적인 한계점이나 극대화되는 효율성'을 다루는 핵심 구절에만 <span class="hl-yellow">...</span> 태그를 지정해주세요.
   - 💡 판단 기준: 단순 서술이 아니라, "실제 기술 면접관이 들었을 때 성능 최적화의 핵심을 이해했다고 판단할 만한 결정적인 포인트 단락(어구)"이어야 합니다. 문장 전체가 길어지면 핵심 단락(약 15자 내외)만 콤팩트하게 끊어서 칠해 공식 가독성을 유지하십시오.5. ➖ 본문 내 대시(-) 문장 구현: 본문에서 세부 특징, 알고리즘 종류, 비교 항목 등을 나열할 때는 그냥 텍스트 줄바꿈을 쓰지 말고, 반드시 <ul>과 <li> 태그를 활용해 정돈된 대시 형태의 흐름을 만들어주세요.
